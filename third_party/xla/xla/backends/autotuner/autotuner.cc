...
absl::StatusOr<Autotuner::Config> Autotuner::ProfileAndPickBest(
  ...
  std::vector<std::pair<Config*, int>> top_configs_and_scratch_bytes;
  ...
  if (autotune_config_.optimize_scratch_bytes) {
    ...
    for (auto& config_and_scratch : top_configs_and_scratch_bytes) {
      if (best_scratch_bytes_config == nullptr ||
          config_and_scratch.second < min_scratch_bytes) {
        best_scratch_bytes_config = config_and_scratch.first;
        min_scratch_bytes = config_and_scratch.second;
      }
    }
    if (best_scratch_bytes_config != nullptr) {
