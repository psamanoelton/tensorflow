...
std::string PrintHloComputationToHtml(
  ...
  // Default indent_amount is 0.

  // ENTRY keyword (default print_computation_mode is kComputationOnly)

  // is_in_nested_computation default is false
  // print_percent  default is true
  printer.Append("%");
  // print_ids default is true
  printer.Append(comp->name());
  printer.Append(" ");

  // print_program_shape default is true
    ...
    for (const HloInstruction* instruction : instruction_order) {
      ...
      std::string diffid =
          it != span_attributes.end() && !it->second.diffid.empty()
              ? absl::StrCat("data-diffid=\"", it->second.diffid, "\"")
              : "";
  ...
  // Default print_name_after_closing_brace is false, so no name is appended.
