...
TEST_F(ZipUtilTest, OpenNonExistentFile) {
  EXPECT_THAT(OpenArchiveWithLibzipTsl("nonexistent.zip").status(),
...
TEST_F(ZipUtilTest, OpenInvalidZipFile) {
  ...
  ASSERT_THAT(tsl::WriteStringToFile(tsl::Env::Default(), invalid_zip_path,
              ...
              IsOk());
  EXPECT_THAT(OpenArchiveWithLibzipTsl(invalid_zip_path).status(),
              StatusIs(absl::StatusCode::kInvalidArgument));
}

TEST_F(ZipUtilTest, GetEntries) {
  auto zip_archive_or = OpenArchiveWithLibzipTsl(zip_path_);
...
TEST_F(ZipUtilTest, OpenRandomAccessFile) {
  ...
  // Read past EOF.
  ASSERT_THAT(raf->Read(0, result, absl::MakeSpan(scratch, 12)), IsOk());
