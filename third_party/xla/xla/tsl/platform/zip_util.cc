...
// Implementation of RandomAccessFile that reads from an in-memory string.
class StringRandomAccessFile : public tsl::RandomAccessFile {
 public:
  ...
  absl::Status Read(uint64_t offset, size_t n, absl::string_view* result,
    ...
    if (offset > data_.size()) {
      return absl::OutOfRangeError(absl::StrCat(
          "Read offset ", offset, " bigger than file size ", data_.size()));
    }
    size_t available = data_.size() - offset;
    size_t bytes_to_read = std::min(n, available);
    if (bytes_to_read > 0) {
      memcpy(scratch, data_.data() + offset, bytes_to_read);
    }
    *result = absl::string_view(scratch, bytes_to_read);
...
class LibzipArchive : public ZipArchive {
 public:
  ...
  absl::StatusOr<std::unique_ptr<tsl::RandomAccessFile>> Open(
    ...
    absl::StatusOr<std::string> content = GetContents(entry);
    if (!content.ok()) {
      return content.status();
    }
    return std::make_unique<StringRandomAccessFile>(*std::move(content));
  }
...
absl::StatusOr<std::unique_ptr<ZipArchive>> OpenArchiveWithLibzipTsl(
  ...
  if (archive == nullptr) {
    ...
    if (!ctx->status.ok()) {
      status_to_return = ctx->status;
    } else if (zip_error_code_zip(&ctx->error) == ZIP_ER_NOZIP) {
      status_to_return = absl::InvalidArgumentError(
          absl::StrCat("File is not a valid zip archive: ", path));
    } else {
      status_to_return = absl::InternalError(
