...
TEST(RuntimeExecutableVersionTest, CurrentRuntimeExecutableVersionIsNotEmpty) {
  TF_ASSERT_OK_AND_ASSIGN(auto client, test_util::GetClient());
  EXPECT_THAT(client->runtime_executable_version(std::nullopt), Not(IsEmpty()));
}
...
TEST(RuntimeExecutableVersionTest,
  ...
  if (runtime_abi_version == "unimplemented") {
    GTEST_SKIP()
