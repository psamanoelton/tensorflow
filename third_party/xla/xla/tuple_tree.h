...
class TupleTree {
 public:
  ...
  class Node {
   public:
    ...
    static Node Tuple(std::vector<Node>&& children) {
      return Node(T(), std::move(children));
    }
    static Node Tuple(const T& value, std::vector<Node>&& children) {
      return Node(value, std::move(children));
    }
    static Node Tuple(T&& value, std::vector<Node>&& children) {
      return Node(std::move(value), std::move(children));
    }
    static Node Tuple(std::initializer_list<Node> children) {
      return Node(T(), std::vector<Node>(children));
    }
    static Node Tuple(const T& value, std::initializer_list<Node> children) {
      return Node(value, std::vector<Node>(children));
    }
    static Node Tuple(T&& value, std::initializer_list<Node> children) {
      return Node(std::move(value), std::vector<Node>(children));
    }
    ...
    static Node FromShape(const Shape& shape, const T& init_value) {
      ...
      return Node::Tuple(init_value, std::move(children));
    }
    ...
    static Node FromShape(const Shape& shape) {
      ...
      return Node::Tuple(T(), std::move(children));
    }
  ...
  explicit TupleTree(absl::Span<U> node_pairs, T default_value = T())
    ...
    for (U& pair : node_pairs) {
      ...
      Node* node =
          GetOrCreateNode(root_node, index,
                          /*preserve_leaf_value=*/false, default_value);
  ...
  void CopySubtreeFrom(const TupleTree<T>& other, const ShapeIndex& src_index,
    ...
    Node* target_node =
        GetOrCreateNode(root_node, dst_index,
                        /*preserve_leaf_value=*/true, default_value_);
  ...
  absl::StatusOr<TupleTree<T>> SubTree(const ShapeIndex& index) const {
    TF_ASSIGN_OR_RETURN(Node node, ToNode(index));
    return TupleTree<T>(std::move(node));
  }
 ...
 private:
  ...
  Node* GetOrCreateNode(Node& root, const ShapeIndex& index,
                        bool preserve_leaf_value, T default_value) {
    Node* node = &root;
    if (index.empty()) {
      return node;
    }
    for (int i = 0; i < index.size(); ++i) {
      ...
      while (idx >= children->size()) {
        children->push_back(Node::Tuple(default_value));
      }
